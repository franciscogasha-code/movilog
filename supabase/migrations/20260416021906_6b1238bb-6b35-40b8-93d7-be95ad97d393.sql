-- Phase 1: Add supplier_pickup to trip_type enum
ALTER TYPE trip_type ADD VALUE IF NOT EXISTS 'supplier_pickup';

-- Phase 1: Add new columns to trips
ALTER TABLE trips ADD COLUMN IF NOT EXISTS destination_description text;
ALTER TABLE trips ADD COLUMN IF NOT EXISTS created_by uuid;

-- Phase 2 + 6: Extend fn_transition_request_status with:
-- 1. flow_type validation for assigned_to_trip (only interurban)
-- 2. Unassignment: assigned_to_trip → in_consolidation (clears trip_id)
...
BEGIN
  ...
  ELSIF v_flow_type = 'interurban' THEN
    v_transition_valid := (
      ...
      (v_old_status = 'assigned_to_trip' AND p_new_status = 'in_consolidation') OR  -- ← NEW: unassignment
  ...
  -- ─── ASSIGNED_TO_TRIP: require trip linkage + flow_type validation ───
  IF p_new_status = 'assigned_to_trip' THEN
    -- NEW: Only interurban orders can be assigned to trips
  ...
  -- ─── UNASSIGNMENT: assigned_to_trip → in_consolidation (clear trip_id) ───
  ...
  ELSIF p_new_status = 'in_consolidation' AND v_old_status = 'assigned_to_trip' THEN
    -- Unassignment: logistics roles only

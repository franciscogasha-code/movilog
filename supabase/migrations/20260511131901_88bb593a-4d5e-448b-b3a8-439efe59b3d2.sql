ALTER TABLE public.branch_request_items
  ADD COLUMN IF NOT EXISTS quantity_unfulfilled numeric NOT NULL DEFAULT 0;

COMMENT ON COLUMN public.branch_request_items.quantity_unfulfilled IS
  'Demanda comercial declarada como no abastecida durante la resolución (no es merma). requested = local_supply_qty + sum(child externals.qty) + quantity_unfulfilled.';
...
BEGIN
  ...
  FOR v_payload_item IN SELECT * FROM jsonb_array_elements(v_payload_items) LOOP
    ...
    IF (v_local_qty + v_sum_externals) > v_item.quantity_requested THEN
      RAISE EXCEPTION 'oversupply_not_allowed: item % requiere % unidades (local=% + externos=%)',
        v_item.id, v_item.quantity_requested, v_local_qty, v_sum_externals;
    END IF;

    v_unfulfilled := v_item.quantity_requested - v_local_qty - v_sum_externals;

    UPDATE public.branch_request_items
       SET local_supply_qty = v_local_qty,
           quantity_unfulfilled = v_unfulfilled
     WHERE id = v_item.id;
    ...
    IF v_unfulfilled > 0 THEN
      v_shortfalls := v_shortfalls || jsonb_build_object(
        ...
        'requested', v_item.quantity_requested,
        'supplied_local', v_local_qty,
        'supplied_external', v_sum_externals,
  ...
  v_summary := jsonb_build_object(
    ...
    'has_shortfall', (jsonb_array_length(v_shortfalls) > 0)
  );
...
BEGIN
  ...
  WHERE pi.request_id = p_parent_id
    AND (
      ...
      + COALESCE(pi.quantity_unfulfilled, 0)
      + COALESCE((

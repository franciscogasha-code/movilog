-- Surgical patch: ensure FO exists & is linked to trip when urban request transitions ready_for_pickup → in_transit
-- This adds a conditional block to fn_transition_request_status (5-arg signature) without altering any other branch.
...
BEGIN
  ...
  -- ============================================================
  -- SURGICAL PATCH: urban request-only pickup → ensure FO + trip link
  -- Only triggers when urban flow goes ready_for_pickup → in_transit
  -- with an active trip provided. Creates FO if missing, links to trip,
  -- and sets driver custody. No effect on any other branch.
  -- ============================================================

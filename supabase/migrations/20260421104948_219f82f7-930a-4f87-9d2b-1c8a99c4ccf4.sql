-- Fix quirúrgico: flujo interurbano cuando origen ya es hub de consolidación
-- Agrega transición ready_for_pickup → in_consolidation EXCLUSIVAMENTE
-- cuando source_branch.is_central_warehouse = true (origen = hub).
-- No toca urbanos, client_delivery, ni el resto del flujo interurbano normal.
...
BEGIN
  ...
  -- Detectar si origen es hub de consolidación (microajuste hub-aware)
  ...
  ELSIF p_new_status = 'in_consolidation' AND v_old_status = 'ready_for_pickup' THEN
    -- Permitir también al origen cuando es hub (no requiere chofer)
  ...
  -- Microajuste hub-aware: si origen es hub e interurbano, auto-avanzar a in_consolidation
  v_auto_consolidate := (
    v_flow_type = 'interurban'
    AND v_source_is_hub
  ...
  IF v_auto_consolidate THEN
    ...
    -- Evento de auto-transición
    v_event_type := 'status_change';
    v_event_description := 'Auto-consolidación: origen es hub (' || v_old_status || ' → ready_for_pickup → in_consolidation)';
    ...
    INSERT INTO public.operational_events (
      ...
      'ready_for_pickup', 'in_consolidation', v_user_id, v_event_category,
...
-- Marcar Depósito Importación (code=5) como hub de consolidación
UPDATE public.branches
SET is_central_warehouse = true
WHERE code = '5' AND name = 'Deposito Importación';
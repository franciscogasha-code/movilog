...
AS $function$
BEGIN
  RETURN public.fn_transition_request_status(
    p_request_id,
    p_new_status,
    p_reason,
    p_rejection_reason_type,
    NULL::uuid
  );
END;
$function$;
...
BEGIN
  ...
  IF v_request.flow_type IS NULL THEN
    IF v_request.delivery_target = 'client_third_party' THEN
  ...
  IF v_auto_consolidate THEN
    ...
    SET status = 'at_hub'::fulfillment_status,
        ...
        updated_at = now()
    WHERE branch_request_id = p_request_id
      AND status::text IN ('pending', 'waiting_for_cut', 'waiting_for_courier', 'picking');

    INSERT INTO public.operational_events (
      reference_id, reference_type, event_type, event_description,
      previous_status, new_status, triggered_by, category, metadata
    ) VALUES (
      p_request_id, 'branch_request', 'status_change',
      'Auto-consolidación: origen es hub (' || v_old_status || ' → ready_for_pickup → in_consolidation)',
      'ready_for_pickup', 'in_consolidation', v_user_id, 'transport'::event_category,
      jsonb_build_object('auto_hub_consolidation', true, 'source_branch_id', v_request.source_branch_id)
    );
  END IF;

  v_event_type := CASE WHEN p_new_status = 'rejected' THEN 'request_rejected' ELSE 'status_change' END;
  v_event_description := 'Transición: ' || v_old_status || ' → ' || p_new_status || COALESCE(' (' || p_reason || ')', '');
  ...
  INSERT INTO public.operational_events (
    reference_id, reference_type, event_type, event_description,
    previous_status, new_status, triggered_by, category
  ) VALUES (
    p_request_id, 'branch_request', v_event_type, v_event_description,
    v_old_status, p_new_status, v_user_id, v_event_category
  );

  RETURN jsonb_build_object(
    'success', true,
    'old_status', v_old_status,
    'new_status', CASE WHEN v_auto_consolidate THEN 'in_consolidation' ELSE p_new_status END,
...
WHERE sb.id = br.source_branch_id
  AND br.status = 'ready_for_pickup'::request_status
  AND br.flow_type = 'interurban'
  AND COALESCE(sb.is_central_warehouse, false) = true;
...
SET status = 'at_hub'::fulfillment_status,
    ...
    current_custody_holder_id = NULL,
    current_location_type = 'branch',
    current_location_branch_id = br.source_branch_id,
    trip_id = NULL,
    updated_at = now()
FROM public.branch_requests br
JOIN public.branches sb ON sb.id = br.source_branch_id
WHERE fo.branch_request_id = br.id
  AND br.status = 'in_consolidation'::request_status
  AND br.flow_type = 'interurban'
  AND COALESCE(sb.is_central_warehouse, false) = true
  AND fo.status::text IN ('pending', 'waiting_for_cut', 'waiting_for_courier', 'picking');
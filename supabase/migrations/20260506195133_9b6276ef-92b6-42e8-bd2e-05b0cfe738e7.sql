...
-- 1. Endurecer fn_send_presale_to_operation: sin fallbacks silenciosos
CREATE OR REPLACE FUNCTION public.fn_send_presale_to_operation(
  p_request_id uuid,
  p_source_branch_id uuid DEFAULT NULL::uuid,
...
BEGIN
  ...
  -- Validaciones explícitas, sin fallback silencioso
...
-- 2. Limpiar datos logísticos en pre-ventas activas (no convertidas)
UPDATE public.branch_requests
SET source_branch_id = NULL,
    delivery_target = NULL,
    shipping_method = NULL,
    flow_type = NULL
WHERE is_pre_sale = true
  AND COALESCE(pre_sale_status, 'draft') IN ('draft','confirmed');

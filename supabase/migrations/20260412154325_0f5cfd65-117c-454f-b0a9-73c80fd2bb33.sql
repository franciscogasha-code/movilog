...
-- 1. Validate source_branch != requesting_branch on insert/update
...
CREATE TRIGGER trg_validate_different_branches
BEFORE INSERT OR UPDATE ON public.branch_requests
FOR EACH ROW
EXECUTE FUNCTION public.fn_validate_different_branches();

-- 2. Update fn_transition_request_status to validate items before leaving pending
...
BEGIN
  ...
  -- ITEM VALIDATION: require at least 1 item to accept (pending → in_preparation)
  IF v_old_status = 'pending' AND p_new_status = 'in_preparation' THEN
    ...
    IF v_item_count = 0 THEN
      RAISE EXCEPTION 'No se puede aceptar un pedido sin ítems. Agregue al menos un producto.';
    END IF;
  END IF;

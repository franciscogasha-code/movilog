-- ============================================================
-- FIX: autorizar transición a 'assigned_to_trip' (vincular carga a viaje)
-- ============================================================
-- CAUSA RAÍZ: la cascada de autorización en fn_transition_request_status
-- no contempla el caso p_new_status = 'assigned_to_trip', por lo que
-- v_actor_allowed queda en FALSE para todos excepto admin/supervisor/owner
-- (que pasan por el atajo v_is_admin).
--
-- El rol técnico existente 'jefe_logistica' YA está incluido en
-- v_is_assignment_operative (junto con warehouse_operator, admin, supervisor, owner),
-- que es el conjunto correcto según la matriz funcional para operaciones
-- de asignación logística (consolidación / asignación de cargas a viajes).
--
-- CAMBIO: se añade una rama ELSIF explícita para 'assigned_to_trip'
-- que reutiliza v_is_assignment_operative. No se crean roles nuevos,
-- no se renombran roles, no se modifican permisos de otros roles ni
-- otras transiciones. Misma firma, misma lógica, solo se cierra el
...
BEGIN
  ...
  -- ⬇️ FIX: rama faltante para vincular carga a viaje.
  -- Reutiliza v_is_assignment_operative (jefe_logistica + warehouse_operator
  -- + admin + supervisor + owner). No crea roles, no altera otros permisos.

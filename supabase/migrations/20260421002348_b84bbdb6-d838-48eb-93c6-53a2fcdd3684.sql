-- Restore full fn_driver_action with all action branches intact.
-- Only surgical change vs original: 'in_consolidation' -> 'in_transit' in pickup and pickup_from_hub
-- (because 'in_consolidation' is not a valid value of fulfillment_status enum).
...
BEGIN
  ...
  IF p_action = 'pickup' THEN
    ...
    -- FIX: 'in_consolidation' no existe en enum fulfillment_status. Usar 'in_transit' siempre.
    -- La distinción de consolidación se mantiene en branch_requests.status vía fn_transition_request_status.
  ...
  ELSIF p_action = 'pickup_from_hub' THEN
    ...
    -- FIX: mismo motivo que pickup. Valor válido del enum fulfillment_status.
    v_new_status := 'in_transit';
    v_new_custody_type := 'driver';
    v_new_custody_holder := v_user_id;
    v_new_location_type := 'vehicle';

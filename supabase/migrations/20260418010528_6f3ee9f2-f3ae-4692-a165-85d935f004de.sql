...
BEGIN
  ...
  ELSIF p_action = 'delivery_failed' THEN
    IF v_old_status NOT IN ('in_consolidation', 'in_transit') THEN
      RAISE EXCEPTION 'No se puede marcar entrega fallida en estado: %', v_old_status;
    END IF;
    ...
    SET status = v_new_status::fulfillment_status,
        ...
        delivery_failed_reason = NULLIF(p_metadata->>'reason', ''),
        updated_at = now()
    WHERE id = p_fulfillment_id;

  ELSIF p_action = 'transfer_to_driver' THEN
    IF v_old_custody_holder IS DISTINCT FROM v_user_id THEN
      RAISE EXCEPTION 'Solo el chofer en custodia puede transferir';
    END IF;

    IF NULLIF(p_metadata->>'target_user_id', '') IS NULL THEN
      RAISE EXCEPTION 'Falta el destinatario de la transferencia';
    END IF;

    SELECT id INTO v_target_driver
    FROM public.drivers
    WHERE user_id = (p_metadata->>'target_user_id')::uuid
      AND COALESCE(is_active, true) = true;

    IF v_target_driver IS NULL THEN
      RAISE EXCEPTION 'El destinatario no es un chofer activo';
    END IF;
  ...
  INSERT INTO public.operational_events (
    ...
    category, previous_status, new_status,
    previous_custody_holder_id, new_custody_holder_id,
    previous_location_branch_id, new_location_branch_id,
    triggered_by, metadata
  ) VALUES (
    'fulfillment_order', p_fulfillment_id, v_event_type, v_event_description,
    v_event_category, v_old_status, v_new_status,

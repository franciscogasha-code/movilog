-- Fix CASO 2: al asignar un pedido a un viaje, garantizar que exista
-- fulfillment_orders con trip_id seteado. Antes sólo se actualizaba si
-- existía (UPDATE ... WHERE branch_request_id = X AND trip_id IS NULL),
-- por lo que pedidos sin FO previa quedaban con request.status =
-- assigned_to_trip pero el viaje mostraba 0 cargas (la lectura usa
-- fulfillment_orders WHERE trip_id = X).
--
-- Reemplazamos sólo la rama p_new_status='assigned_to_trip' para insertar
-- la FO si no existe, sin alterar el resto de la lógica/permiso.
...
BEGIN
  ...
  IF p_new_status = 'assigned_to_trip' THEN
    ...
    -- ⬇️ FIX raíz CASO 2: si el pedido aún no tiene fulfillment_order,
    -- crear una en estado at_hub vinculada al viaje. Antes el UPDATE
    -- siguiente no afectaba a ninguna fila y el viaje mostraba 0 cargas.
    SELECT EXISTS (

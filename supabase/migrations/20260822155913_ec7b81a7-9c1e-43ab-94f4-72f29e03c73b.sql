...
BEGIN
  ...
  ELSIF current_setting('movilog.saneamiento', true) = 'on' AND (
      (v_old_status = 'in_consolidation' AND p_new_status = 'in_transit') OR
      (v_old_status IN ('in_supply', 'supplied', 'delivered_to_third_party', 'ready_for_delivery', 'delivered') AND p_new_status = 'rejected')
    ) THEN
    -- EXCLUSIVO saneamiento de lanzamiento (flag movilog.saneamiento, solo lo fija
    -- fn_saneamiento_lanzamiento, función service_role): interurbanos sin viaje con
    -- transferencia BIMS como evidencia del movimiento, y rechazo de estados sin
    -- salida operativa. La matriz operativa normal no cambia.
    v_transition_valid := TRUE;
...
BEGIN
  ...
  -- Flags exclusivos del saneamiento:
  --  movilog.saneamiento: habilita en la RPC las transiciones excepcionales gateadas
  --  movilog.supply_internal: bypass de fn_block_supply_transitions para
  --    in_supply/supplied -> rejected (pedidos sin transferencia BIMS)
  PERFORM set_config('movilog.saneamiento', 'on', true);
  ...
  LOOP
    ...
    -- Criterio central: transferencia BIMS vinculada = evidencia del movimiento
    SELECT EXISTS (
      SELECT 1 FROM public.request_bims_documents d
      WHERE d.request_id = v_req.id AND d.document_type = 'transfer'
    ) INTO v_has_transfer;

    v_flow := COALESCE(v_req.flow_type, 'interurban');

    -- DRY-RUN: clasificar y prever huecos sin escribir nada
    IF p_dry_run THEN
      IF v_has_transfer AND v_req.status IN ('in_supply','supplied','ready_for_delivery','delivered_to_third_party') THEN
        v_huecos := v_huecos || jsonb_build_object(
          'request_id', v_req.id,
          'request_number', v_req.request_number,
          'status', v_req.status,
          'motivo', 'PREVISTO: CON transferencia pero en estado sin camino de cierre hacia adelante'
        );
        CONTINUE;
      END IF;
      IF v_has_transfer AND v_req.status = 'in_preparation' THEN
        SELECT COUNT(*) INTO v_items FROM public.branch_request_items WHERE request_id = v_req.id;
        IF v_items = 0 THEN
          v_huecos := v_huecos || jsonb_build_object(
            'request_id', v_req.id,
            'request_number', v_req.request_number,
            'status', v_req.status,
            'motivo', 'PREVISTO: pedido sin ítems, la RPC no permite avanzar a ready_for_pickup'
          );
          CONTINUE;
        END IF;
      END IF;
      ...
      v_key := v_req.status
        || CASE WHEN v_has_transfer THEN ' [CON transfer] -> closed' ELSE ' [SIN transfer] -> rejected' END;
      v_counts := jsonb_set(v_counts, ARRAY[v_key], to_jsonb(COALESCE((v_counts ->> v_key)::int, 0) + 1));
      IF v_has_transfer THEN v_closed := v_closed + 1; ELSE v_rejected := v_rejected + 1; END IF;
      CONTINUE;
    END IF;

    -- RUN REAL
    BEGIN
      SELECT br.status::text INTO v_current FROM public.branch_requests br WHERE br.id = v_req.id;
      IF v_current IS DISTINCT FROM v_req.status THEN
        IF v_current IN ('closed', 'rejected') THEN
        ...
        ELSE
          v_huecos := v_huecos || jsonb_build_object(
            ...
            'request_number', v_req.request_number,
            'status', v_req.status,
            'motivo', 'El estado cambio durante el run (ahora: ' || COALESCE(v_current, 'NULL') || ')'
          );
        END IF;
        CONTINUE;
      END IF;
      ...
      IF NOT v_has_transfer THEN
        PERFORM public.fn_transition_request_status(
          v_req.id, 'rejected', 'saneamiento_lanzamiento', 'other', NULL
        );
        INSERT INTO public.operational_events (
          ...
          'Saneamiento lanzamiento (sin transferencia BIMS): ' || v_current || ' -> rejected',
          v_current, 'rejected', p_actor,
          jsonb_build_object('motivo', 'saneamiento_lanzamiento')
        );
        v_rejected := v_rejected + 1;
      ELSE
        -- Loop dinámico: relee el estado real en cada paso (absorbe el
        -- auto-salto ready_for_pickup -> in_consolidation de origenes hub)
        v_iter := 0;
        LOOP
          SELECT br.status::text INTO v_current FROM public.branch_requests br WHERE br.id = v_req.id;
          EXIT WHEN v_current IN ('closed', 'rejected') OR v_iter >= 10;

          v_next := CASE v_current
            WHEN 'in_preparation' THEN 'ready_for_pickup'
            WHEN 'ready_for_pickup' THEN CASE WHEN v_flow = 'interurban' THEN 'in_consolidation' ELSE 'in_transit' END
            WHEN 'in_consolidation' THEN 'in_transit'
            WHEN 'assigned_to_trip' THEN 'in_transit'
            WHEN 'in_transit' THEN 'delivered'
            WHEN 'delivered' THEN 'received'
            WHEN 'received' THEN 'logistic_closed'
            WHEN 'logistic_closed' THEN 'closed'
            ELSE NULL
          END;

          IF v_next IS NULL THEN
            RAISE EXCEPTION 'Estado % sin camino de cierre en saneamiento', v_current;
          END IF;

          PERFORM public.fn_transition_request_status(
            v_req.id, v_next, 'saneamiento_lanzamiento', NULL, NULL
          );
          INSERT INTO public.operational_events (
            category, reference_id, reference_type, event_type, event_description,
            previous_status, new_status, triggered_by, metadata
            ...
            'closure', v_req.id, 'branch_request', 'saneamiento_transition',
            'Saneamiento lanzamiento (con transferencia BIMS): ' || v_current || ' -> ' || v_next,
            v_current, v_next, p_actor,
            jsonb_build_object('motivo', 'saneamiento_lanzamiento')
          );
          v_iter := v_iter + 1;
        END LOOP;
        ...
        IF v_current <> 'closed' THEN
          RAISE EXCEPTION 'No llego a closed (quedo en %)', v_current;
        END IF;
    ...
    EXCEPTION WHEN OTHERS THEN
      v_huecos := v_huecos || jsonb_build_object(
        ...
        'request_number', v_req.request_number,
        'status', v_req.status,
        'transferencia_bims', v_has_transfer,
        'motivo', SQLERRM
      );
    END;
  END LOOP;
  ...
  v_report := jsonb_build_object(
    ...
    'cutoff', p_cutoff,
    'actor', p_actor,
    'criterio', 'CON transferencia BIMS -> closed paso a paso; SIN transferencia -> rejected',
    'conteos', v_counts,
    'cerrarian_o_cerrados', v_closed,
    'rechazarian_o_rechazados', v_rejected,
  ...
  VALUES (
    ...
    'branch_requests',
    CASE WHEN p_dry_run THEN 'DRY-RUN V2 criterio-transferencia (sin escritura)' ELSE 'RUN REAL V2 criterio-transferencia' END,
    v_report
  );

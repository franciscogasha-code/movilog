...
import { Plus, Search, MessageCircle, Clock, CheckCircle2, ArrowRight } from "lucide-react";
    ...
    <motion.div className="space-y-6" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Consultar stock antes de crear solicitudes</p>
        </div>
                ...
                <tbody>
                  {consultations.map((c: any) => (
                    <tr key={c.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors cursor-pointer" onClick={() => setSelectedId(c.id)}>
                      <td className="p-3 font-medium">{c.product?.name} <span className="text-muted-foreground text-xs">({c.product?.sku})</span></td>
...
function ConsultationForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!productId || !branchId || !targetBranches.length) {
    ...
    try {
      ...
      const { data: consultation, error } = await supabase
        .from("availability_consultations")
        .insert({ product_id: productId, requesting_branch_id: branchId, created_by: user.id })
        .select()
        .single();
      if (error) throw error;
...
function ConsultationDetail({ consultationId }: { consultationId: string }) {
  const { data: consultation } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("availability_consultations")
        .select(`*, product:products(name, sku), requesting_branch:branches!availability_consultations_requesting_branch_id_fkey(name, code)`)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="font-display text-lg font-bold">{c.product?.name}</h3>
          <p className="text-sm text-muted-foreground">{c.product?.sku} — desde {c.requesting_branch?.code}</p>
        </div>

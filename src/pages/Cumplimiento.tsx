...
export default function Cumplimiento() {
  ...
  return (
    <motion.div className="space-y-6" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
      <div>
        <h1 className="font-display text-2xl font-bold text-foreground">Cumplimiento Físico</h1>
        <p className="text-muted-foreground mt-1">Órdenes de fulfillment: picking, despacho, tránsito y recepción</p>
      </div>
      ...
      <Card className="glass-card">
        <CardHeader className="pb-3">
          <CardTitle className="font-display text-lg">Órdenes de Cumplimiento</CardTitle>
        </CardHeader>
        <CardContent className="p-0">
          {isLoading ? (
            <div className="p-8 text-center text-muted-foreground">Cargando...</div>
          ) : !orders?.length ? (
            <div className="p-8 text-center text-muted-foreground">No hay órdenes de cumplimiento</div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    ...
                    <th className="text-left p-3 font-medium text-muted-foreground">Próximo evento</th>
                ...
                <tbody>
                  {orders.map((o: any) => (
                    <tr key={o.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="p-3 text-muted-foreground text-xs">{SHIPPING_METHOD_LABELS[o.shipping_method]}</td>
                      <td className="p-3">
                        <StatusBadge status={o.status} config={FULFILLMENT_STATUS_CONFIG} />
                      </td>
                      <td className="p-3 text-xs text-muted-foreground">{o.expected_next_event || "—"}</td>
